...
export const EditorialHero: React.FC = () => {
  ...
  const { data: heroData, isLoading } = useCMSContent('editorial-hero');
  
  // Safely extract CMS fields (using fallbacks for unpopulated backend)
  const fields = heroData?.data || heroData?.fields || heroData || {};
  const rawImage = fields.hero_image;
  const heroImage = typeof rawImage === 'string' ? rawImage : rawImage?.url || '';
  const heroTitle = fields.hero_title || 'CAMPAIGN PENDING';
  const heroSubtitle = fields.hero_subtitle || 'Awaiting editorial upload from the Atelier.';
    ...
    <section aria-label="Campaign Hero Showcase" className="w-full relative bg-background">
      ...
      <div className="w-full max-w-[1920px] mx-auto border-b border-border">
        ...
        <div className="relative w-full h-[65vh] sm:h-[75vh] md:h-[82vh] lg:h-[88vh] overflow-hidden group bg-neutral-900">
          {isLoading ? (
            <Skeleton className="w-full h-full rounded-none bg-neutral-800" />
          ) : (
            <>
              <ImageWithFallback
                src={heroImage}
